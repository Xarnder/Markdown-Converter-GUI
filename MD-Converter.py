...
@contextmanager
def prepared_markdown_file(md_path: Path):
    """Yield a markdown path safe for Pandoc, sanitizing broken inline images if needed."""
    ...
    if sanitized_text == markdown_text:
        yield md_path
        return
...
def apply_docx_table_borders(docx_path: Path) -> None:
    """Add solid grid borders to every table in a Word document."""
    import shutil
    import zipfile
    from xml.etree import ElementTree as ET
...
def markdown_file_to_export_html(md_path: Path) -> str:
...
def convert_md_to_pdf(md_path: Path, pdf_path: Path) -> None:
    ...
    if pandoc_available():
        ...
        html = markdown_file_to_export_html(md_path)
...
def convert_to_markdown(file_path: Path, output_path: Path, md_engine: MarkItDown) -> None:
...
def convert_file(
    ...
    if mode == "to_markdown":
        ...
        convert_to_markdown(file_path, output_path, md_engine)
    elif mode == "to_docx":
        convert_md_to_docx(file_path, output_path)
    elif mode == "to_pdf":
        convert_md_to_pdf(file_path, output_path)
        ...
        errors: list[str] = []
        try:
            convert_md_to_docx(file_path, docx_path)
